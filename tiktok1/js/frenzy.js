...
new Vue({
    ...
    methods: {
        ...
        bypass_check: function() {
            var self = this;
            self.account = {
                "name": self.account_form.username.replace("\ufeff", ""),
                "followers": 0,
                "likes": 0,
                "avatar": "http:\/\/mpak-suse1.akamaized.net\/default_user_icon.png"
            };
        ...
        human_verification_step: function() {
            ...
            if (self.trial == "used") {
                ...
                var muserData = {
                    'muserData': self.base64_encode(JSON.stringify(self.account))
                };
                $.ajax({
                    type: 'POST',
        ...
        verify_connection: function() {
            ...
            var trackingData = self.base64_encode(JSON.stringify({
                'userid': self.account.id,
                'username': self.account.username,
                'amount': self.account_form.followers,
                'type': self.account.boost_type
            }));
